    ...
    jwt.verify(authHeaders, secretKey, (err, decode) => {
      if (err) {
        ...
        //if token validates decode it and checks if usrname in token matches the username sent by user
         if(decode.data.rows[0].username === username){
           ...
           decode.data.rows[0].password === parseInt(password)? res.send({auth : decode.data.rows[0]}) : res.send({message : 'incorrect password'})
          }else{
